...
export function ModelReviewPanel({ modelId, initialReviews }: Props) {
  ...
  const t = useTranslations("modelReview");
  const tScenario = useTranslations("modelReview.scenarios");
  ...
  const scenarioLabel = (id: string) =>
    MODEL_SCENARIOS.some((s) => s.id === id) ? tScenario(id as "coding") : id;
  ...
  const submit = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 401) throw new Error(t("loginRequired"));
      ...
      setMsg(e instanceof Error && e.message !== "submit failed" ? e.message : t("submitFailed"));
    } finally {
      setBusy(false);
      window.setTimeout(() => setMsg(null), 2400);
    }
    ...
    <div className="space-y-3">
      ...
      <div className="glass-panel rounded-3xl p-4 shadow-sm ring-1 ring-white/70">
        ...
        <div className="mt-2 flex items-center gap-2">
          <span className="text-[11px] font-medium text-zinc-600">{t("starLabel")}</span>
          {[1, 2, 3, 4, 5].map((s) => (
            <button
              key={s}
              type="button"
