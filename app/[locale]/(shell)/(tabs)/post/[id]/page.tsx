...
export default async function PostPage({ params }: Props) {
  ...
  if (!post) notFound();
  if (post.status === "draft" && post.authorId !== viewerId) notFound();
  ...
  const label = isPostType(post.type)
    ? getPostTypeLabel(tPost, post.type)
    : post.type;
        ...
        <div className="space-y-4 p-4">
          ...
          {recruitMeta ? (
            <div className="rounded-2xl bg-violet-50/80 p-3 text-xs ring-1 ring-violet-200/60">
              <p className="font-semibold text-violet-950">{t("recruitInfo")}</p>
              <ul className="mt-2 space-y-1 text-violet-900/90">
                {recruitMeta.recruitRole ? (
                  <li>{t("recruitRole", { value: recruitMeta.recruitRole })}</li>
                ) : null}
                {recruitMeta.recruitTime ? (
                  <li>{t("recruitTime", { value: recruitMeta.recruitTime })}</li>
                ) : null}
                {recruitMeta.recruitComp ? (
                  <li>{t("recruitComp", { value: recruitMeta.recruitComp })}</li>
                ) : null}
              </ul>
            </div>
          ) : null}
          ...
          {!isDraft ? <PostComments postId={post.id} viewerId={viewerId} /> : null}
